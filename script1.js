const PAGES = [{
    ...
    const BIRTHDAYS = [
        { month: "Січень", people: [
            { name: "Данильченко Руслан", day: 10 },
            { name: "Вовкодав Наталія", day: 26 },
            { name: "Мазур Валентина", day: 30 }
        ]},
        { month: "Лютий", people: [
            { name: "Брижан Валерій", day: 5 },
            { name: "Поліщук Ігор", day: 6 },
            { name: "Смаженко Борис", day: 24 }
        ]},
        { month: "Березень", people: [
             { name: "Миронець Ірина", day: 3 }
        ]},
        { month: "Квітень", people: [
            { name: "Кондратюк Володимир", day: 5 },
            { name: "Батурінцев Олесандр", day: 10 },
            { name: "Кобець Володимир", day: 10 },
            { name: "Корсун Валентина", day: 12 },
            { name: "Ткаченко Володимир", day: 16 },
            { name: "Стаканова Тетяна", day: 30 }
        ]},
        { month: "Травень", people: [
            { name: "Максюта Анжела", day: 6 },
            { name: "Білоконь Оксана", day: 20 },
            { name: "Скородзієвський Олександр", day: 24 },
            { name: "Кравчук Альона", day: 26 },
            { name: "Кравцова Лілія", day: 27 }
        ]},
        { month: "Червень", people: [
            { name: "Тельман Людмила", day: 4 },
            { name: "Черток Олена", day: 6 },
            { name: "Домнін Володимир", day: 20 },
            { name: "Чорна Лілія", day: 30 },
            { name: "Денисюк Ігор", day: 30 },
        ]},
        { month: "Липень", people: [
           { name: "Колос Олександр", day: 11 }
        ]},
        { month: "Серпень", people: [
            { name: "Біла Ірина", day: 11 },
            { name: "Шкляренко Валентина", day: 15 },
            { name: "Богачук Надія", day: 21 }
        ]},
        { month: "Вересень", people: [
           { name: "Кметь Любов", day: 9 },
            { name: "Ерган Світлана", day: 23 },
            { name: "Свид Юрій", day: 27 }
        ]},
        { month: "Жовтень", people: [
            { name: "Винник Віктор", day: 1 },
            { name: "Костюк Олександр", day: 2 },
            { name: "Лихенко Тетяна", day: 2 },
            { name: "Рудченко Ольга", day: 8 }
        ]},
        { month: "Листопад", people: [
            { name: "Зінкевич Олена", day: 28 }
        ]},
        { month: "Грудень", people: [
            { name: "Чорна Любов", day: 1 }
        ]}
    ];
    ...
    function renderPage(pageId) {
      ...
      if (page.isBirthdays) {
        renderBirthdaysPage(page);
      } else if (page.isRadio && page.streams && page.streams.length > 0) {
        ...
        stationLabel.htmlFor = "stationSelector";
        stationLabel.className = "radio-info";
        stationLabel.textContent = "Виберіть радіостанцію:";

        const stationSelector = document.createElement("select");
        stationSelector.id = "stationSelector";
        stationSelector.className = "station-selector";
        ...
        audioPlayer.src = page.streams[0].url;

        stationSelector.addEventListener("change", () => {
          audioPlayer.src = stationSelector.value;
          audioPlayer.load();
          audioPlayer.play().catch(e => console.error("Autoplay was blocked or an error occurred:", e));
        });

        playerContainer.appendChild(stationLabel);
        playerContainer.appendChild(stationSelector);
    ...
    function renderBirthdaysPage(page) {
        const pageDiv = document.createElement("div");
        pageDiv.className = "page";
        ...
        container.className = "birthdays-container";

        BIRTHDAYS.forEach(monthData => {
            if (monthData.people.length > 0) {
                ...
                const sortedPeople = monthData.people.sort((a, b) => a.day - b.day);
                ...
                sortedPeople.forEach(person => {
                    ...
                    dateSpan.textContent = person.day;
    ...
    function showModal() {
      ...
      if (page.localVideos && page.id === "local-videos") {
        ...
        photoCounter.textContent =
          `Відео ${currentIndex + 1} / ${page.localVideos.length}`;
        ...
        modalCaption.textContent = page.name;
        photoCounter.textContent =
          `${currentIndex + 1} / ${page.photos.length}`;
    ...
    function prevMedia() {
      ...
      let globalIndex = slideshowPlaylist.findIndex(
        (item) =>
      ...
      globalIndex =
        (globalIndex - 1 + slideshowPlaylist.length) % slideshowPlaylist.length;
...
function checkBirthdays() {
    ...
    const monthData = BIRTHDAYS.find(m => m.month === currentMonthName);

    if (!monthData) return;

    const birthdayPeople = monthData.people.filter(p => p.day === currentDay);

    if (birthdayPeople.length > 0) {
        const notificationElement = document.getElementById('birthday-notification');
        const namesElement = document.getElementById('birthday-names');
        const birthdaySound = document.getElementById('birthdayAudio');

        const namesString = birthdayPeople.map(p => p.name).join(', ');
        
        namesElement.textContent = namesString;
        notificationElement.classList.remove('hidden');

        if (!sessionStorage.getItem('birthdayGreetingPlayed')) {
            const playBirthdayMusic = () => {
                if (birthdaySound && birthdaySound.paused) {
                    birthdaySound.play().then(() => {
                        sessionStorage.setItem('birthdayGreetingPlayed', 'true');
                        document.removeEventListener('click', playBirthdayMusic);
                    }).catch(error => {
                        console.log("Відтворення заблоковано:", error);
                    });
                }
            };
            ...
            document.addEventListener('click', playBirthdayMusic, { once: true });
        }
    }
}
