...
function fadeImage(index) {
  ...
  // Після того, як зображення зникне, ми змінимо його джерело
  setTimeout(() => {
    // Ця функція виконається, коли нове зображення буде завантажено
    const showNewImage = () => {
      modalImg.alt = caption;
      modalCaption.textContent = page.name;
      photoCounter.textContent = `${index + 1} / ${page.photos.length}`;
      // Плавно показуємо нове зображення
      modalImg.style.opacity = 1;
      // Прибираємо обробник, щоб він не спрацьовував для наступних зображень
      modalImg.removeEventListener('load', showNewImage);
    };

    // Додаємо обробник події 'load' ПЕРЕД зміною src
    modalImg.addEventListener('load', showNewImage);

    // Змінюємо src, що запускає завантаження
    modalImg.src = src;

    // На випадок, якщо зображення вже в кеші і подія 'load' може не спрацювати,
    // викликаємо функцію вручну
    if (modalImg.complete) {
      showNewImage();
    }
  }, 300); // Ця затримка має відповідати часу анімації зникнення в CSS
}
