...
function changeTrack() {
  ...
  if (musicPlaying) {
    audio.play().catch(() => { /* ігноруємо помилку */ });
  }
}

// Функції лічильника років (винесені з анонімної функції)
const graduationYear = 1987;
const graduationDate = new Date(graduationYear, 5, 30);
// Елементи DOM для лічильника років можуть бути не одразу доступні.
// Отримаємо їх під час DOMContentLoaded.
let yearsNumberEl;
let yearsCounterEl;
let yearsSuffixEl;
...
function initYearsCounter() {
  // Ініціалізація DOM-елементів для лічильника років
...
// Обробник подій DOMContentLoaded, що викликає всі початкові функції
document.addEventListener("DOMContentLoaded", () => {
  showPage(PAGES[0].id);
  populateMusicSelector();
  updateVisitCounter();
  initYearsCounter(); // Тепер initYearsCounter викликається після того, як DOM готовий
  
  // Додаємо обробник події 'change' для musicSelector
  if (musicSelector) {
