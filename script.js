const PAGES = [
  ...
  /* NEW: Локальні відео
       Щоб додати свої відео: скопіюй файли у папку `video/`
       і додай шляхи сюди, наприклад: 'video/clip1.mp4' */
  {
    id: "local-videos",
    name: "Відео",
    localVideos: [
...
function showPage(pageId) {
  ...
  if (page.photos) {
    ...
    // Render local video thumbnails / players
    const gallery = document.createElement("div");
    gallery.className = "gallery";
    page.localVideos.forEach((videoPath, index) => {
      // Create a poster-like video element (muted) for preview
      ...
      // optional: small inline preview on hover/play; keep simple: click to open modal
      vid.onclick = () =>
      ...
      // hide native controls in grid to keep look consistent
      vid.controls = false;
      vid.style.cursor = "pointer";
      vid.style.width = "100%";
      vid.style.borderRadius = "10px";
      vid.style.objectFit = "cover";
      // show a tiny poster frame (browsers often show first frame)
...
// helper: показ/приховування елементів photo-only
function setPhotoControlVisibility(visible) {
  const els = document.querySelectorAll(".photo-only");
  els.forEach((el) => {
    // для кнопок/селектів просто сховаємо або відновимо
...
function openModal(src, caption, index, photos) {
  ...
  if (page.videoUrl) {
    ...
    isVideoMode = true;
    // ховаємо елементи, які видимі лише для фото
    setPhotoControlVisibility(false);
  } else {
    modalImg.style.display = "block";
    modalVideo.style.display = "none";
    // hide local modal video if visible
    if (modalLocalVideo) {
    ...
    isVideoMode = false;
    // показуємо елементи, призначені для фото
...
// NEW: Open modal for a local video
...
function closeModal() {
  ...
  // опціонально: відновлюємо видимість керувань фото (на випадок, якщо наступне відкриття — фото)
  setPhotoControlVisibility(true);
}
...
function advanceToNextAlbum() {
  ...
  // шукаємо наступний альбом з фото
  let iterations = 0;
  while (
