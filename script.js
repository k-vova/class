...
function showPage(pageId) {
    ...
    if (page.id === "youtube-videos") {
        if (page.youtubeVideos && page.youtubeVideos.length > 0) {
            ...
            // --- ПОЧАТОК ЗМІН ---
            // 1. Створюємо головний контейнер і надаємо йому клас 'page', як у фотоальбомів
            const pageContainer = document.createElement('div');
            pageContainer.className = 'page';

            // 2. Створюємо заголовок і надаємо йому клас 'album-title' для однакового стилю
            const title = document.createElement("h3");
            title.className = "album-title";
            title.textContent = page.name;
            pageContainer.appendChild(title);

            // 3. Створюємо контейнер для відео
            const videoContainer = document.createElement("div");
            videoContainer.className = "video-page-container";
            // Прибираємо зайві відступи, оскільки вони вже є у батьківського елемента 'page'
            ...
            // 4. Додаємо єдиний стилізований контейнер на сторінку
...
function openLocalVideoModal(src, caption, index, videos) {
    ...
    modalLocalVideo.play().catch((e) => console.error("Video play failed:", e));
    closeBtn.focus();
}
...
function prevVideo() {
    ...
    currentIndex =
        (currentIndex - 1 + page.localVideos.length) % page.localVideos.length;
    updateLocalVideo();
}
...
function nextImage() {
    ...
    if (currentIndex >= page.photos.length) {
        // --- ПОЧАТОК ВИПРАВЛЕННЯ ---
        // Перевіряємо, чи це останній альбом у списку слайдшоу
        if (currentPageId === 'meet2022-25') {
            // Якщо так, переходимо до першого альбому "Дитячий садочок"
            const firstPage = PAGES[0]; // Перший альбом в масиві
            showPage(firstPage.id);
            currentIndex = 0;
            openModal(
                normalizePhoto(firstPage.photos[currentIndex], `Фото 1`).src,
                normalizePhoto(firstPage.photos[currentIndex], `Фото 1`).caption,
                currentIndex,
                firstPage.photos
            );
            return; // Завершуємо виконання, щоб не викликати advanceToNextAlbum()
        }
        // --- КІНЕЦЬ ВИПРАВЛЕННЯ ---

        // Інакше, переходимо до наступного альбому, як зазвичай
        advanceToNextAlbum();
        return;
    }
    fadeImage(currentIndex);
...
function advanceToNextAlbum() {
    ...
    let iterations = 0;
    while (
        !PAGES[nextIndex].photos &&
        !PAGES[nextIndex].localVideos &&
    ...
    if (nextPage.photos && nextPage.photos.length > 0) {
        ...
        isVideoMode = false;
    } else if (nextPage.localVideos && nextPage.localVideos.length > 0) {
        showPage(nextPage.id);
        currentIndex = 0;
        openLocalVideoModal(
            nextPage.localVideos[currentIndex],
            nextPage.name,
            currentIndex,
            nextPage.localVideos
        );
        isVideoMode = true;
    }
}
